...

{
    ...
    "title": "Docker",
    "category": "section",
    "text": "Change directory to MAVs  Build image:  sh build.shThe shell script pull the base image from avpg/cain and then run the Dockerfile to build mavs.  Run image:  sh run.shThe shell script runs the image as container with volumes setup. For simplicity, volumes are the mechanism for persisting data generated by and used by containers.   When using container to run demonstration, you can store the data in MAVs/shared_dir and still be able to access it after closing the container. Besides, any change in the MAVs/ros/src of local machine will change instantly in the container. That is, you can update lanuch files, parameters files, Python scripts without building image again. However, if you modify cpp files, then you need to rebuild the image to generate the execution files.Launch the docker from other terminals:  docker ps -l # Show the latest created container\ndocker exec -it <container_name> /bin/bash"
},
